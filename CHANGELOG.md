...
- **可恢复的远期会话档案**：监听 DSH 的 `compaction/summary`，在压缩完成时把退出近期窗口的
  用户原话与助手最终正文幂等回填到 L0，再按 12 小时桶生成独立语义梗概。梗概只负责定位，
  不混入 L1 长期事实；`conversation_search` 可先检索梗概，再以 `archive_id` 分页取回该段原文。
  fork 沿用当前分支加祖先的可见域，兄弟分支严格隔离；档案引用的 L0 行不受常规保留期清理。
  压缩主路径只等待本地 JSONL/SQLite 落盘，LLM 摘要在后台串行完成，失败保留 pending 下次重试。

### 修复

- **召回工具指南导致状态快照抖动**：此前 `memory:profile` 以“上一轮是否实际注入 L1”
  为门控，召回命中与去重压制交替时会让 `@deepseek-ai/dsh-system-prompt` 快照反复出现、
  消失。现在工具选择与合计搜索上限直接归入 tools schema，不再把指南重复注入召回消息
  或动态 system prompt；稳定区只保留确有内容的 L2/L3 画像与场景导航。
