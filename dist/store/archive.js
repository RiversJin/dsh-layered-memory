...
export function truncateArchiveSummary(text, maxChars) {
...
export function packArchiveSegments(sessionId, messages, maxChars, now = Date.now()) {
...
export class ArchiveStore {
    ...
    queued = new Set();
    disposed = false;
    constructor(dataDir, db, l0, ctx, cfg, logger) {
        this.db = db;
        this.l0 = l0;
        this.ctx = ctx;
        this.cfg = cfg;
    ...
    async init() {
        ...
        const files = (await fs.readdir(this.dir).catch(() => [])).filter((name) => name.endsWith('.jsonl')).sort();
        for (const name of files) {
            const records = await readJsonl(path.join(this.dir, name));
            for (const record of records) {
                if (record?.id && record.sessionId && Array.isArray(record.messageIds))
                    this.db.upsertArchive(record);
            }
        }
        for (const record of this.db.listArchivesNeedingSummary(ARCHIVE_SUMMARY_VERSION))
            this.enqueue(record);
    }
    /**
    ...
    async ingest(sessionId, messages) {
        if (this.disposed || messages.length === 0)
            return { backfilled: 0, segments: 0 };
        const cfg = this.cfg();
        ...
        for (const candidate of packArchiveSegments(sessionId, messages, cfg.archive.maxSegmentChars)) {
            ...
            if (!this.db.upsertArchive(record))
                continue;
    ...
    search(query, limit, visibleSessionIds, strict = false) {
        if (limit <= 0)
            return [];
    ...
    enqueue(record) {
        if (this.disposed || this.queued.has(record.id))
            return;
    ...
    async summarize(record) {
        ...
        if (!this.db.upsertArchive(ready))
