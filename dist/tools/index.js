...
export function registerMemoryTools(ctx, cfg, stores, logger, modes, live, lineage) {
    ...
    const presetOf = (agentId) => agentId === undefined ? undefined : lineage?.presetOf(agentId);
    ...
    ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            const scope = rawScope === 'global' || rawScope === 'preset' || rawScope === 'branch'
                ? rawScope
            ...
            const candidates = await stores.l1.searchCandidates(content, Math.max(stores.l1.size, 20), family, visible, callerPreset);
            const norm = (s) => s.replace(/\s+/g, ' ').trim().toLocaleLowerCase();
            const duplicate = candidates.find((r) => r.type === type && (r.scope ?? 'global') === scope &&
            ...
            if (args.replaces?.trim()) {
                ...
                const targetVisible = !!target && (targetScope === 'global' ||
    ...
    ctx.tools.register(defineTool({
        ...
        description: '搜索当前会话的原始历史（L0）和压缩档案梗概，返回带时间戳的消息。先按 query 找档案；自动召回给出 archive id 后可传 archive_id 读取对应原文。适合具体原话、时间线与上下文细节；结构化偏好、事实、任务或规则优先用 memory_search。两种搜索每轮合计最多调用 3 次；仍无结果时直接按已有信息回答。',
        parameters: {
        ...
        execute: async (args, exec) => {
            if (familyOfCaller(exec.agent?.id) === null)
                return { items: [], archives: [], notice: blockNoticeOf(exec.agent?.id) };
            const limit = Math.min(Math.max(args.limit ?? 5, 1), 20);
